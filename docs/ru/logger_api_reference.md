...
Когда для проекта включены Telegram-уведомления, каждое сообщение начинается с строки `<Название проекта> (<UUID>)`, чтобы администратор мог мгновенно понять источник инцидента.
