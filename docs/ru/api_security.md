...
- Сообщение `ADD_<UUID>` подписывает чат на проект после проверки UUID и наличия проекта. Бот уведомляет пользователя, предотвращает дубли и сбрасывает счётчик ошибок.【F:api/src/telegram/notifier.ts†L226-L259】
- Сообщение `DELETE_<UUID>` отменяет подписку либо сообщает, что чат ещё не привязан к проекту.【F:api/src/telegram/notifier.ts†L261-L292】
