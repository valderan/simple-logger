...
Поле `maxLogEntries` управляет объёмом хранилища для конкретного проекта. Если указано значение больше нуля, API перестаёт принимать новые записи при достижении лимита. Для Logger Core ограничение всегда равно нулю (без ограничения).
...
Команды Telegram и изменения получателей через API теперь дополняют системные события полями `metadata.chatId`, `metadata.userId`, `metadata.projectUuid`, а если чат привязан сразу к нескольким проектам — массивом `metadata.projectSubscriptions`. Это позволяет быстро определить, в каком диалоге и в рамках какого проекта возникла проблема.

Если структура лога не соответствует ожидаемой, событие записывается в системный проект `logger-system`.

Ответы, которые может вернуть эндпоинт приёма логов:

- `201 Created` — запись сохранена, в ответе приходит документ и поле `rateLimitPerMinute`.
- `400 Bad Request` — данные не прошли валидацию.
- `403 Forbidden` — внешняя попытка записать лог в `logger-system`; событие фиксируется как инцидент безопасности.
- `404 Not Found` — проект не найден.
- `409 Conflict` — превышен лимит `maxLogEntries`. В Logger Core автоматически появляется критическая запись с тегами `LOG_CAP` и `ALERT`.
