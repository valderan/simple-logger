...
- Ping-мониторинг HTTP-сервисов с ручным запуском проверок и записью падений в Logger Core.
- Telegram-оповещения по тегам и инцидентам с явным указанием названия и UUID проекта в каждом сообщении.
...
- Автоматическое добавление в системные логи Telegram полей `chatId`, `userId`, `projectUuid` и списка подписок (`projectSubscriptions`) для быстрой диагностики инцидентов.
